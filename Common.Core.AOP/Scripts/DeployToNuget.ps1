...
$projectFileName = "Common.Core.AOP.csproj"
...
if($isUploadToNuget){	
	# CMD to Push Nuget
	$cmdline = "dotnet nuget push .\bin\Release\IdeaActivator.Common.Core.AOP.$version.nupkg --api-key $nugetApiKey --source https://api.nuget.org/v3/index.json"
	Write-Host $cmdline
	CMD.exe /c $cmdline
}else{
